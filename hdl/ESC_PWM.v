...
// Counter widths are derived from the terminal counts rather than hand-picked.
// A counter narrower than its compare value can never reach it and wraps at its
// own width instead.
 localparam PWM0_TOP = 240000;   // 12 MHz / 240000 = 50 Hz   (servo)
 localparam PWM1_TOP = 24000;    // 12 MHz /  24000 = 500 Hz  (ESC)
 localparam PWM0_W   = $clog2(PWM0_TOP + 1);
 localparam PWM1_W   = $clog2(PWM1_TOP + 1);

 reg[PWM0_W-1:0] counter_PWM0=0;// counter for creating 50 Hz PWM signal
 reg[PWM1_W-1:0] counter_PWM1=0;// counter for creating 500 Hz PWM signal


 reg[PWM0_W-1:0] DUTY_CYCLE0=60000;
 reg[PWM1_W-1:0] DUTY_CYCLE1=6000;
...
// Create 50 Hz PWM signal with variable duty cycle controlled by 2 buttons 
 ...
 always @(posedge clk)
 begin
// Step and ceiling are sized against the ESC frame (PWM1_TOP = 24000), not the
// servo frame. A ceiling carried over from the servo channel would span most of
// an entire ESC period, so one button press would saturate the channel.
...
// PWM1_TOP is the 500 Hz terminal count. A counter too narrow to reach it wraps
// at its own width instead -- at 17 bits, that is 131072 counts and 91.55 Hz.
// tb/esc_pwm_measure_tb.v measures both outputs at a true 12 MHz.
   if(counter_PWM1>=PWM1_TOP)
    counter_PWM1 <= 0;
 end
